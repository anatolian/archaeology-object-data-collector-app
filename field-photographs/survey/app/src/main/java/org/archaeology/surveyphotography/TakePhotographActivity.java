...
public class TakePhotographActivity extends Activity
{
	...
	private File getOutputMediaFile(String fieldOrBag)
    {
		...
		{
			mediaStorageDir = new File(Environment.getExternalStorageDirectory().getPath().toString()
                    + photoSavePath + suYear + "/" + suSeqNum + "/fld");
			thumbMediaStorageDir = new File(Environment.getExternalStorageDirectory().getPath().toString()
