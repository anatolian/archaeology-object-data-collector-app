...
public class PicassoWrapper
{
    public class ResizeAccordingToSpecificHeightTransformation implements Transformation
    {
        ...
        public Bitmap transform(Bitmap source)
        {
            ...
            {
                Log.v(LOGTAG, "result height is smaller than source height");
                return source;
            }
            else
            {
                ...
                Log.v(LOGTAG, "ratio : " + ratio);
                Log.v(LOGTAG, "source size: " + source.getByteCount());
                Log.v(LOGTAG, "result size: " + result.getByteCount());
                return result;
            }
        }
     ...
     * @param loadedPhotos           - target images loaded
     */
    public void fetchAndInsertImage(LinearLayout photoLayout, final Uri remoteImageURL,
                                    ...
                                    final ArrayList<TaggedImageView> loadedPhotos)
    {
        Log.v(LOGTAG, "Fetach and Insert Image function called for image " + remoteImageURL);
        ...
        Picasso.with(aContext).load(remoteImageURL).transform(new ResizeAccordingToSpecificHeightTransformation())
