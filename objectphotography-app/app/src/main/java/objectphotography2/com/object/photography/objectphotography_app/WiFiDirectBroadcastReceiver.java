...
public class WiFiDirectBroadcastReceiver extends BroadcastReceiver
{
    interface WifiDirectBroadcastReceivable
    {
        ...
        /**
         * Disable get IP button
         */
        void disableGetIpButton();
        ...
        /**
         * Set if message should appear
         * @param connectDialogAppeared - whether message appeared
         */
        void setConnectDialogAppeared(boolean connectDialogAppeared);
    ...
    public WifiP2pManager.ConnectionInfoListener mConectionListener = new WifiP2pManager.ConnectionInfoListener() {
        /**
         * Info found
        ...
        {
            Log.v(LOGTAG_WIFIDIRECT, "onConnectionInfoAvailable: WifiP2pInfo: " + info.toString());
            // InetAddress from WifiP2pInfo struct.
            NetworkInfo networkInfo = (NetworkInfo) myIntent.getParcelableExtra(WifiP2pManager.EXTRA_NETWORK_INFO);
            if (networkInfo != null)
            {
                Log.v(LOGTAG_WIFIDIRECT, "networkInfo: " + networkInfo.toString());
            }
            ...
            if (info.groupFormed && info.isGroupOwner)
            {
                ...
                Log.v(LOGTAG, "the group has been formed and you found the owner");
            ...
            else if (info.groupFormed)
            {
                ...
                Log.v(LOGTAG, "the group has been formed but you cannot find the owner");
                connectedToDevice = true;
                mActivity.enableGetIpButton();
            }
        }
    ...
    private WifiP2pManager.PeerListListener peerListListener = new WifiP2pManager.PeerListListener() {
        ...
        public void onPeersAvailable(WifiP2pDeviceList peerList)
        {
            ...
            if (peers.size() == 0)
            {
                Log.d(LOGTAG_WIFIDIRECT, "No devices found");
            }
            else
            {
    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        Log.v(LOGTAG, "the action is "+action);
        Log.v(LOGTAG, "you are in the receive method");
        Log.v(LOGTAG, "have not gone into state changed if condition");
        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action))
        {
            int state = intent.getIntExtra(WifiP2pManager.EXTRA_WIFI_STATE, -1);
            Log.v(LOGTAG, "the state is "+String.valueOf(state));
            if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED)
            {
                // Wifi P2P is enabled
                mActivity.enableDiscoverPeersButton();
                Log.v(LOGTAG, "wifi state is enabled");
            }
            else
            {
                mActivity.disableDiscoverPeersButton();
                Log.v(LOGTAG, "wifi state is not enabled");
                // Wi-Fi P2P is not enabled
            }
            // Check to see if Wi-Fi is enabled and notify appropriate activity
        }
        else if (WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action)) {
            Log.v(LOGTAG, "inside peers changed loop");
            if (!mActivity.isConnectDialogAppeared())
            {
                Log.v(LOGTAG, "connect dialog did not appear");
                if (mManager != null)
                {
                    Log.v(LOGTAG, "requesting peers from mywifiactivity");
            ...
            else
            {
                ...
                Log.v(LOGTAG, "peers changed and setting info listener");
                mActivity.enableGetIpButton();
                int state = intent.getIntExtra(WifiP2pManager.EXTRA_WIFI_STATE, -1);
                Log.v(LOGTAG, "the state is "+String.valueOf(state));
                if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED)
        ...
        {
            Log.v(LOGTAG, "you are inside connection changed action");
            if (mManager == null)
            {
                Log.v(LOGTAG, "mManager was equal to null");
                return;
            }
            NetworkInfo networkInfo = (NetworkInfo) intent.getParcelableExtra(WifiP2pManager.EXTRA_NETWORK_INFO);
            Log.v(LOGTAG, String.valueOf(networkInfo.describeContents()));
            if (networkInfo.isConnected())
            {
                // We are connected with the other device, request connection info to find group owner IP
                Log.v(LOGTAG, "you are connected with the device");
                mManager.requestConnectionInfo(mChannel, mConectionListener);
            }
            ...
            Log.v(LOGTAG_WIFIDIRECT, "Wifi Direct Connection Status Changed");
